...
const bird: Bird = { x: 260, y: H / 2, vy: 0 };

const spriteCanvas = document.createElement('canvas');
spriteCanvas.width = 1050;
spriteCanvas.height = 400;
const sctx = spriteCanvas.getContext('2d') as CanvasRenderingContext2D;
sctx.imageSmoothingEnabled = false;

function buildSprite(): SpriteBox {
...
function drawCloudLobe(cx: number, cy: number, r: number): void {
  ctx.moveTo(cx + r, cy);
...
async function loadBest(): Promise<void> {
  try {
    const res = await (window as any).storage.get('flappy-code-best', false);
...
async function saveBest(value: number): Promise<void> {
  try {
    await (window as any).storage.set('flappy-code-best', String(value), false);
  } catch (e) {}
}
...
function update(): void {
  ...
  clouds.forEach(c => {
    c.x -= c.speed;
    if (c.x < -60) { c.x = W + 60; c.y = 30 + Math.random() * 260; }
  });
...
function drawBird(): void {
  ...
  ctx.drawImage(
    spriteCanvas,
    (spriteCanvas.width - spriteBox.boxW) / 2,
    (spriteCanvas.height - spriteBox.boxH) / 2,
    spriteBox.boxW, spriteBox.boxH,
    -dw / 2, -dh / 2, dw, dh
  );
  ctx.restore();
}
